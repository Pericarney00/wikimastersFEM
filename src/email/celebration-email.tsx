...
export default async function sendCelebrationEmail(
  ...
  // SELECT usersSync.id, usersSync.email FROM articles LEFT JOIN uersSync ON articles.authorID = userSync.id WHERE articles.id = $id
  ...
  const emailRes = await resend.emails.send({
    ...
    to: "pericarney@gmail.com", // unless you set up your own domain, you can only email yourself
    subject: `✨ You article got ${pageviews} views! ✨`,
    react: (
      <CelebrationTemplate
        articleTitle={title}
