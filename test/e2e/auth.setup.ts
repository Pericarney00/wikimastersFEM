...
setup("authenticate", async ({ page }) => {
  ...
  try {
    // Click on "Email & Password" tab to show password field
    const emailPasswordTab = page.locator('text=Email & Password');
    ...
    // Verify we're logged in by checking that Sign In button is no longer visible
    await expect(page.locator('text=Sign In')).not.toBeVisible({ timeout: 5000 });
